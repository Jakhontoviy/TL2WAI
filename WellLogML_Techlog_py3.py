...
class WellLogMLGenerator:
    ...
    def _is_html_content(text: str) -> Tuple[bool, str]:
        ...
        if text_stripped.startswith('<') and ('>' in text_stripped):
            # Проверить наличие распространённых HTML тегов
        ...
        # Признак 2: Очень большой текст (> 1000 символов) с признаками HTML структуры
        if len(text_stripped) > 1000:
            ...
            if indicators_found >= 2:  # Если найдено 2+ HTML индикатора
        ...
        try:
            ...
            for prop_name in prop_list:
                ...
                try:
                    ...
                    # Проверить наличие HTML содержимого и пропустить его
                    is_html, reason = self._is_html_content(str(prop_value) if prop_value else '')
                    if is_html:
                        self._log(f"      Свойство '{prop_name}' пропущено (HTML содержимое): {reason}", 'info')
                        continue
            ...
            try:
                ...
                if index_data is not None:
                    ...
                    index_curve_info = {
                        ...
                        "variableData": [float(val) for val in index_data]
                    }
            ...
            if prop_list:
                for prop_name in prop_list:
                    ...
                    try:
                        ...
                        is_html, reason = self._is_html_content(str(prop_value) if prop_value else '')
                        if is_html:
                            self._log(f"        Датасет '{dataset_name}': свойство '{prop_name}' пропущено (HTML): {reason}", 'debug')
                            continue
            ...
            if prop_list:
                for prop_name in prop_list:
                    ...
                    try:
                        ...
                        is_html, reason = self._is_html_content(str(prop_value) if prop_value else '')
                        if is_html:
                            self._log(f"        Переменная '{variable_name}': свойство '{prop_name}' пропущено (HTML): {reason}", 'debug')
                            continue
            ...
            try:
                ...
                for ds_idx, dataset_name in enumerate(datasets, 1):
                    ...
                    for var_idx, variable_name in enumerate(variables_to_export, 1):
                        try:
                            # Получить тип переменной для специальной обработки аннотаций
                            try:
                                var_type = db.variableType(well_name, dataset_name, variable_name)
                            except Exception:
                                var_type = 'Continu'
                            ...
                            # Попытка 1: Обычная загрузка данных
                            try:
                                curve_data = db.variableLoad(well_name, dataset_name, variable_name)
                            except Exception as e:
                                logger.debug(f"        {variable_name}: ошибка в variableLoad(): {e}")

                            # Попытка 2: Для аннотаций и RichText — загрузить из свойств
                            if (curve_data is None or (isinstance(curve_data, (list, np.ndarray)) and len(curve_data) == 0)):
                                if var_type in ['Annotation', 'RichText', 'TopBottomCurve']:
                                    logger.info(f"        {variable_name} ({var_type}): variableLoad() вернул пусто, проверяем свойства...")
                                    try:
                                        prop_list = db.variablePropertyList(well_name, dataset_name, variable_name)
                                        logger.info(f"        {variable_name}: доступные свойства: {prop_list}")

                                        # Пробуем различные подходы для загрузки значения
                                        # Подход A: Прямо пробуем все свойства
                                        for prop_name in (prop_list if prop_list else []):
                                            try:
                                                ann_value = db.variablePropertyValue(well_name, dataset_name, variable_name, prop_name)
                                                if ann_value is not None and ann_value != '':
                                                    value_str = str(ann_value)
                                                    # Пропускаем UUID-подобные значения (ID свойства)
                                                    if not (len(value_str) == 24 and all(c in '0123456789abcdef' for c in value_str.lower())):
                                                        curve_data = [ann_value]
                                                        logger.info(f"        {variable_name}: загружено из свойства '{prop_name}' = {ann_value}")
                                                        break
                                            except Exception as e:
                                                logger.debug(f"        {variable_name}: ошибка при чтении свойства '{prop_name}': {e}")

                                        # Подход B: Если ничего не загрузилось, попытаться загрузить переменную как целое
                                        if curve_data is None:
                                            try:
                                                # Может быть это одиночное значение, а не массив
                                                var_value = db.variableLoad(well_name, dataset_name, variable_name)
                                                if var_value is not None:
                                                    logger.info(f"        {variable_name}: повторная попытка variableLoad() вернула: {var_value} (тип: {type(var_value).__name__})")
                                            except Exception as e:
                                                logger.debug(f"        {variable_name}: ошибка при повторной загрузке: {e}")
                                    except Exception as e:
                                        logger.warning(f"        {variable_name}: ошибка при обработке свойств: {e}")

                            if curve_data is not None:
                                # Важно: для строк не использовать np.array() напрямую - разбит на символы!
